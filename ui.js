function renderSidebar(active, perfil) {
  ...
  const titulo = visibles.find(l=>l.key===active)?.label || '';
  ...
  aside.id = 'sidebar';
  aside.innerHTML = `
    <div class="sidebar-brand"><span class="logo-dot"></span>TenshoGym <span class="sub">CRM</span></div>
    <nav class="sidebar-nav">
      <div class="sidebar-label">Menú</div>
      ${visibles.map(l=>`<a href="${l.href}" class="sidebar-link${l.key===active?' active':''}"><span class="icon">${l.icon}</span>${l.label}</a>`).join('')}
    </nav>
    <div class="sidebar-footer">
      ${perfil?`<div class="sidebar-user"><div class="avatar">${iniciales}</div><div class="info"><div class="name">${perfil.nombre}</div><div class="role">${perfil.rol==='admin'?'Administrador':'Recepción'}</div></div></div>`:''}
      <div class="flex-gap" style="gap:8px">
        <button id="theme-toggle-btn" class="theme-toggle" onclick="toggleTheme()">☀️</button>
        <button class="btn btn-danger btn-sm" style="flex:1" onclick="cerrarSesion()">Salir</button>
      </div>
    </div>`;
  document.body.insertBefore(aside, document.body.firstChild);

  // Topbar (va DENTRO de .main-content, ANTES de .page-content)
  const pageContent = document.querySelector('.page-content');
  if (pageContent) {
    const topbar = document.createElement('div');
    topbar.className = 'topbar';
    topbar.innerHTML = `<button class="sidebar-toggle" onclick="toggleSidebar()">☰</button><span class="topbar-title">${titulo}</span><div></div>`;
    pageContent.parentElement.insertBefore(topbar, pageContent);
  }
